...
public class PlayerController : MonoBehaviour {
	...
	void Start() {
		...
		gunTrans = transform.Find ("GunPos").transform;

		//Gets player UI elements that will not be rotated
		staticCanvas = transform.Find ("StaticPlayerCanvas");
		healthBar = staticCanvas.Find ("Healthbar").Find("Health").GetComponent<RectTransform>();
		ammoCount = staticCanvas.Find ("AmmoCount").GetComponent<Text>();

		//Gets player UI elements that will rotate (noAmmoTxt's child is an exception)
		rotatableCanvas = transform.Find ("RotatablePlayerCanvas");
		noAmmoTxt = rotatableCanvas.Find ("OutOfAmmoText").gameObject;
		powerArrow = rotatableCanvas.Find ("PowerArrow").GetComponent<RectTransform> ();
	}
